...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < letters.length; i += 1) {
    ...
    const filter = letters.filter((char) => char === letter);
    if (filter.length === 1) {
      result = letter;
      break;
    }
...
function isCreditCardNumber(ccn) {
  ...
  const check = digits.slice(0, -1).reverse().map((val, ind) => {
    let result = val;
    if (ind % 2 === 0) {
      let newVal = val * 2;
      if (newVal > 9) {
  ...
  return 10 - (check % 10) === lastDigit || (check % 10 === 0 && lastDigit === 0);
}
...
function getDigitalRoot(num) {
  ...
  const result = num.toString().split('').map(Number).reduce((sum, dig) => sum + dig);
...
function getCommonDirectoryPath(pathes) {
  const getCommonSubstr = (str1, str2) => {
    let result = '';
  ...
  for (let i = 1; i < pathes.length; i += 1) {
    result = getCommonSubstr(pathes[i], result);
  }
  const lastIndexOfSlash = result.lastIndexOf('/');
  return lastIndexOfSlash !== -1 ? result.slice(0, result.lastIndexOf('/') + 1) : result;
}
...
function getMatrixProduct(m1, m2) {
  const resultHeight = m1.length;
  const resultWidth = m2[0].length;
  const width = m1[0].length;

  const result = (new Array(resultHeight)).fill(0).map(() => (new Array(resultWidth)).fill(0));

  for (let i = 0; i < resultHeight; i += 1) {
    for (let j = 0; j < resultWidth; j += 1) {
